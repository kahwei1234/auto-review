...
describe('[IBK - Motor Insurance] Vehicle Details - ISM Incomplete Info | Form',async () => {
    ...
    it('[TC02] ID Type: MyKAD - Verify submit button and x button for missing info page',async () => {
        ...
        //Wait
        await browser.pause(1000);
        const {chassisnumber} = Missing_Info_MyKad_1
        ...
        allureReporter.addStep("Insert engine number");
        const {enginenumber} = Missing_Info_MyKad_1
        await MissingInfoPage.engine_number.setValue(enginenumber)

        allureReporter.addStep("Click x button");
        //Wait
        await browser.pause(1000);
        await MissingInfoPage.x_button.click()
