...
import {Invalid_More_details_required_MyKad_1} from '../../../../test-data/MotorInsurance.js'
...
describe('[IBK - Motor Insurance] Refer Risk ',async () => {
    ...
    it('[TC02b] Verify not now button for more details required page',async () => {
        ...
        const {vehicleregistrationnumber} = Invalid_More_details_required_MyKad_1
        ...
        const {idnumber} = Invalid_More_details_required_MyKad_1
        ...
        const {postcode} = Invalid_More_details_required_MyKad_1
        ...
        await expect(MoreDetailsRequiredPage.submit_button).toHaveAttr('disabled')

        allureReporter.addStep("Click not now button");
        //Wait
        await browser.pause(1000);
        await MoreDetailsRequiredPage.not_now_button.click()
